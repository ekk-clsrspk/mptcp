#!/bin/bash
###############################################################################
#  MPTCP Router Deployment Script
#  Run on: Router (Ubuntu, 5 WAN + 1 LAN)
#  Usage:  sudo bash router.sh "<SHADOWSOCKS_PASSWORD>"
#
...
# WAN interfaces — order matters: index matches subnet number
# WAN1 = 192.168.1.2 on enp6s18, WAN2 = 192.168.2.2 on enp6s19, etc.
WAN_INTERFACES=("enp6s18" "enp6s19" "enp6s20" "enp6s21" "enp6s22")
WAN_IPS=("192.168.1.2" "192.168.2.2" "192.168.3.2" "192.168.4.2" "192.168.5.2")
WAN_GATEWAYS=("192.168.1.1" "192.168.2.1" "192.168.3.1" "192.168.4.1" "192.168.5.1")
WAN_SUBNETS=("192.168.1.0/24" "192.168.2.0/24" "192.168.3.0/24" "192.168.4.0/24" "192.168.5.0/24")
...
BRUTAL_UP=5000       # 5 × 1Gbps upload
BRUTAL_DOWN=10000    # 5 × 2Gbps download
MUX_CONNECTIONS=8    # parallel mux streams
...
if [[ $# -lt 1 || -z "${1:-}" ]]; then
    err "Usage: sudo bash router.sh \"<SHADOWSOCKS_PASSWORD>\""
